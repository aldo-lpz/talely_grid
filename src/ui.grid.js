...
$.widget("talely.grid", {
  ...
  _reorder_array: function() {
    ...
    this._content.find("tr").each(function(i, row) {
      ...
      return row.attr("tg-row-index", i);
    });
    return new_order;
  },
  ...
  _create_column: function(col, row, index) {
    ...
    switch (col.type) {
      ...
      case "checkbox":
        ...
        break;
      case "controls":
        c.append(this._add_controls_column(index, col.name, row[col.name]));
        break;
      case "select":
  ...
  _add_controls_column: function(index, name, value) {
    var control, ref, select, state, _i, _len, _ref;
    ref = this;
    select = $(document.createElement('select'));
    _ref = EAD.external.dev_tools.controls;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      control = _ref[_i];
      state = value.type === control.val ? "selected='selected'" : "";
      select.append("<option value='" + control.val + "' " + state + ">" + control.name + "</option>");
    }
    select.bind("change", function(event) {
      var selected;
      selected = $(this).val();
      control = _.find(EAD.external.dev_tools.controls, function(curr) {
        return curr.val === selected;
      });
